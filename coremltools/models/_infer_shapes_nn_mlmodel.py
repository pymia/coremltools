...
def _permute(layer, shape_dict):
    ...
    Hout = dims[axis[2]]
    Wout = dims[axis[3]]
    shape_dict[layer.output[0]] = (
        int(Seq_out), Batch, int(Cout), int(Hout), int(Wout))
...
def infer_shapes(nn_spec, input_spec, input_shape_dict=None):
        ...
        for inp in input_spec:
            ...
            if inp.type.WhichOneof('Type') == 'imageType':
                ...
                if colorspace == 'GRAYSCALE':
                    ...
                    raise ValueError(
                        'Input %s : Invalid Colorspace' % (input_name))
    ...
    for i, layer in enumerate(layers):
        for inp in layer.input:
            assert inp in shape_dict, ('Input %s shape not cannot be determined' % (
                inp))
