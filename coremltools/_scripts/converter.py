    ...
    if args.srcModelFormat == 'caffe':
        ...
        try:
            model = converters.caffe.convert(model,
                                             image_input_names=set(
                                                 args.imageInputNames),
                                             is_bgr=args.isBGR,
        ...
        try:
            ...
            model = converters.keras.convert(model,
                                             ...
                                             args.outputNames,
                                             image_input_names=set(
                                                 args.imageInputNames) if args.imageInputNames else None,
                                             is_bgr=args.isBGR,
                                             red_bias=args.redBias,
                                             blue_bias=args.blueBias,
                                             green_bias=args.greenBias,
...
def _main():
    ...
    parser.add_argument('--srcModelFormat', type=unicode, choices=[
                        'auto', 'caffe', 'keras'], default='auto', help='Format of model at srcModelPath (default is to auto-detect).')
    parser.add_argument('--srcModelPath', type=unicode, required=True,
                        help='Path to the model file of the external tool (e.g caffe weights proto binary, keras h5 binary')
    parser.add_argument('--dstModelPath', type=unicode, required=True,
                        help='Path to save the model in format .mlmodel')
    parser.add_argument('--caffeProtoTxtPath', type=unicode, default='',
                        help='Path to the .prototxt file if network differs from the source file (optional)')
    parser.add_argument('--meanImageProtoPath', type=unicode, default='',
    ...
    parser.add_argument('--classInputPath', type=unicode, default='',
                        help='Path to class labels (ordered new line separated) for treating the neural network as a classifier')
    parser.add_argument('--predictedFeatureName', type=unicode, default='class_output',
